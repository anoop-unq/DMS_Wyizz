...
import { FiBell } from "react-icons/fi";
import { HiOutlineMenu } from "react-icons/hi";
import { useAuthContext } from "../context/AuthContext"; // ✅ 1. Import Added

export default function Navbar({
  setSidebarToggle,
  isMobile = false,
  // You can still keep this prop as a fallback if needed
  user: propUser = { name: "Marhaba Admin", role: "Admin", initials: "M" },
}) {
  // ✅ 2. Get dynamic data from Context
  const { userType, user: contextUser } = useAuthContext();

  // ✅ 3. Construct the final user object
  // We prioritize Context data; if missing, we fall back to the prop/default
  const currentUser = {
    name: contextUser?.name || propUser.name,
    // Use userType from context, capitalize it if needed (e.g. "admin" -> "Admin")
    role: userType 
      ? userType.charAt(0).toUpperCase() + userType.slice(1) 
      : propUser.role,
    initials: contextUser?.name
      ? contextUser.name.split(" ").map((s) => s[0]).slice(0, 2).join("")
      : propUser.initials,
  };
