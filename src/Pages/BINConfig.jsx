...
const BINConfig = ({
  ...
  // ✅ Updated Payload: Automatically sets true if all items are checked
  ...
  const generatePayload = (
    ...
    return currSegments.map((segName) => {
      ...
      const isAllBins =
        masterRanges.length > 0 &&
        selectedRanges.length === masterRanges.length;

      const isAllTokens =
        masterTokens.length > 0 &&
        selectedTokens.length === masterTokens.length;
      ...
      return {
        ...
        discount_apple_tokens: selectedTokens.map((v) => ({ token_value: v })),

        discount_bins: binPayload.length > 0 ? binPayload : undefined,
      };
    });
  };
  ...
  // ✅ All Ranges Toggle Logic

  const handleToggleAllRanges = (segName, masterRanges) => {
    const current = segmentRanges[segName] || [];

    const next =
      current.length === masterRanges.length ? [] : [...masterRanges];

    const updated = { ...segmentRanges, [segName]: next };

    setSegmentRanges(updated);

    updateParent({ segmentRanges: updated });
  };

  // ✅ All Tokens Toggle Logic
  ...
  const handleToggleAllTokens = (segMetaTokens) => {
    ...
    const isAllChecked = tokenValues.every(
      (v) => tokens.find((tk) => tk.value === v)?.checked
    );
  ...
  const mapApiDataToState = (discountData) => {
    const segmentsApi =
      discountData.discount_segments || discountData.segments || [];
    ...
    segmentsApi.forEach((segment) => {
      ...
      const masterSegment = availableSegments.find(
        (s) => (s.segment_name || s.name) === segName
      );

      let masterRanges = masterSegment?.bin_ranges?.map(formatBinRange) || [
        "N/A - N/A",
      ];
      ...
      const apiTokens =
        segment.discount_apple_tokens || segment.apple_tokens || [];

      apiTokens.forEach((token) => {
        const val = token.token_value || token;

        if (!newTokens.some((t) => t.value === val))
          newTokens.push({ value: val, checked: true });
      });
    });
  ...
  useEffect(() => {
    const fetchSegments = async () => {
      ...
      try {
        ...
        setAvailableSegments(res.data?.rows || res.data || []);
      } catch (err) {
        console.error(err);
      } finally {
        setLoadingSegments(false);
    ...
    if (campaignId && availableSegments.length > 0) {
      const fetchStepData = async () => {
        ...
        try {
          ...
          if (d.discount_segments?.length > 0) mapApiDataToState(d);
        } catch (err) {
          console.error(err);
        } finally {
          setIsLoadingData(false);
  ...
  const toggleSegment = (segment) => {
    ...
    let newNames = isSelected
      ? selectedSegments.filter((s) => s !== name)
    ...
    if (isSelected) delete newRanges[name];
      ...
      newRanges[name] = segment.bin_ranges?.length
        ? segment.bin_ranges.map(formatBinRange)
        : ["N/A - N/A"];

    const newTokens = [...tokens];

    if (!isSelected) {
      (segment.apple_tokens || segment.discount_apple_tokens || []).forEach(
        (t) => {
          const v = t.token_value || t;

          if (!newTokens.some((nt) => nt.value === v))
            newTokens.push({ value: v, checked: true });
        }
      );
    }
  ...
  const handleSubmit = async (action) => {
    if (selectedSegments.length === 0)
      return alert("Select at least one segment.");
    ...
    try {
      ...
      await campaignDiscountApi.update(campaignId, {
        discount: { discount_segments: payload },
      });
  ...
  return (
    <div className="bg-white rounded-lg border border-gray-200 p-4 shadow-sm">
      ...
      <div className="bg-[#F7F9FB] border border-[#E2E8F0] rounded p-4 mb-6">
        <div ref={segRef} className="relative w-full">
          <label className="block text-sm text-gray-700 mb-2 font-medium">
            Select Segment Name <span className="text-red-500">*</span>
          </label>
          ...
          <div className="flex gap-4">
            <button
              type="button"
              onClick={() => setSegmentsOpen(!segmentsOpen)}
              className="flex-1 flex items-center justify-between border border-[#B0B2F7] rounded p-2 bg-white text-sm h-10 overflow-hidden text-left"
            >
            ...
            <button
              type="button"
              onClick={() => setSegmentsOpen(true)}
              className="px-8 bg-[#7747EE] text-white rounded-full text-sm h-9"
            >
              Add
            </button>
          </div>
            ...
            <div className="absolute z-30 mt-2 w-full bg-white border border-[#E2E8F0] rounded shadow-md p-3 max-h-48 overflow-auto hide-scroll">
              {availableSegments.map((seg) => {
                ...
                return (
                  <label
                    key={seg.id}
                    className="flex items-center gap-3 py-2 text-sm cursor-pointer select-none"
                  >
                    <input
                      type="checkbox"
                      checked={isChecked}
      ...
      <div
        className={`grid grid-cols-1 lg:grid-cols-2 gap-6 ${
          selectedSegments.length > 4
            ? "max-h-[700px] overflow-y-auto pr-2 hide-scroll"
            : ""
        }`}
      >
        {selectedSegments.map((segName) => {
          ...
          const availableRanges = originalSeg?.bin_ranges?.map(
            formatBinRange
          ) || ["N/A - N/A"];

          const isAllRanges =
            (segmentRanges[segName] || []).length === availableRanges.length;

          const isAllTokens =
            segMetaTokens.length > 0 &&
            segMetaTokens.every(
              (t) =>
                tokens.find((tk) => tk.value === (t.token_value || t))?.checked
            );

          return (
            <div
              key={segName}
              className="bg-white border border-gray-100 rounded p-4 relative shadow-sm h-fit"
            >
              <button
                onClick={() => toggleSegment(originalSeg)}
                className="absolute top-3 right-3 w-4 h-4 rounded-full bg-[#7747EE] text-[8px] flex items-center justify-center text-[#ffffff] z-10"
              >
                ✕
              </button>
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 border-t border-[#E2E8F0] pt-4">
                  ...
                  <div className="flex items-center justify-between mb-3 pr-2 border-b border-[#E2E8F0] pb-2">
                    <label className="flex items-center gap-2 cursor-pointer select-none">
                      ...
                      <input
                        ...
                        checked={isAllRanges}
                        onChange={() =>
                          handleToggleAllRanges(segName, availableRanges)
                        }
                      />
                      ...
                      >
                        <svg
                          className={`${
                            isAllRanges ? "opacity-100" : "opacity-0"
                          } w-3 h-3 text-white`}
                          fill="none"
                          stroke="currentColor"
                          strokeWidth="3"
                  ...
                  <div
                    className={`space-y-3 ${
                      availableRanges.length > 5
                        ? "max-h-[160px] overflow-y-auto pr-2 hide-scroll"
                        : ""
                    }`}
                  >
                    {availableRanges.map((r) => {
                      const isRChecked = (
                        segmentRanges[segName] || []
                      ).includes(r);
                      ...
                      return (
                        ...
                        >
                          <input
                            ...
                            checked={isRChecked}
                            onChange={() => {
                              const curr = segmentRanges[segName] || [];

                              const next = isRChecked
                                ? curr.filter((x) => x !== r)
                                : [...curr, r];
                          ...
                          >
                            <svg
                              className={`${
                                isRChecked ? "opacity-100" : "opacity-0"
                              } w-3 h-3 text-white transition-opacity`}
                              fill="none"
                              stroke="currentColor"
                              strokeWidth="3"
                ...
                <div>
                  ...
                  <div className="flex items-center justify-between mb-3 pr-2 border-b border-[#E2E8F0] pb-2">
                    <label className="flex items-center gap-2 cursor-pointer select-none">
                      ...
                      <input
                        ...
                        className="sr-only"
                        checked={isAllTokens}
                        onChange={() => handleToggleAllTokens(segMetaTokens)}
                      />
                      ...
                      >
                        <svg
                          className={`${
                            isAllTokens ? "opacity-100" : "opacity-0"
                          } w-3 h-3 text-white`}
                          fill="none"
                          stroke="currentColor"
                          strokeWidth="3"
                  ...
                  <div
                    className={`space-y-3 ${
                      segMetaTokens.length > 5
                        ? "max-h-[120px] overflow-y-auto pr-2 hide-scroll"
                        : ""
                    }`}
                  >
                    {segMetaTokens.length ? (
                      segMetaTokens.map((t) => {
                        const val = t.token_value || t;

                        const isTChecked = tokens.find(
                          (tk) => tk.value === val
                        )?.checked;
                        ...
                        return (
                          ...
                          >
                            ...
                            >
                              <svg
                                className={`${
                                  isTChecked ? "opacity-100" : "opacity-0"
                                } w-3 h-3 text-white transition-opacity`}
                                fill="none"
                                stroke="currentColor"
                                strokeWidth="3"
                            ...
                            <div className="text-[11px] text-gray-700 truncate font-medium leading-none">
                              {val}
                            </div>
                          </label>
                        );
      ...
      <div className="mt-8 border-t border-[#E2E8F0] pt-4 flex justify-between items-center bg-white">
        <button
          onClick={onPrevious}
          className="bg-white border border-[#E2E8F0] rounded-[5px] px-6 py-[5px] text-[#334155] text-[14px] font-medium hover:bg-gray-50 transition-colors"
        >
          ← Previous
        </button>
        ...
        <div className="flex gap-3">
          {isEditMode && (
            <button
              ...
              className="px-6 py-2 bg-green-600 text-white rounded text-sm font-medium hover:bg-green-700 transition-colors"
            >
              Update
            </button>
          )}
          ...
          <button
            ...
            className="bg-[#6366F1] rounded-[5px] px-8 py-[5px] text-[#ffffff] text-[14px] font-medium hover:bg-[#4f46e5] transition-colors"
          >
            Next →
          </button>
        </div>
