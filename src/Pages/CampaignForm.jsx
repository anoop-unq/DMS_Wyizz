...
export default function CampaignForm({
  ...
  useEffect(() => {
    ...
    if (initialData) {
      ...
      // ✅ FIX: Extract Segment Names only (Strings), not full objects
      ...
      const newFormData = {
        ...
        step2: {
          // ✅ FIX IS HERE: Assigning array of strings
          selectedSegments: segmentNames, 
          selectedSegmentIds: segmentIds,
          segmentRanges: {}, // Logic for ranges needs API enrichment usually
          tokens: [], // Logic for tokens
        },
        step3: { ranges: [] },
        step4: {
      ...
      <div className="mb-6">
        {step === 1 && (
          <CampaignDetails
            ...
            onRefresh={onRefresh} // Pass refresh prop
...
const Stepper = ({ steps, currentStep, onStepClick }) => (
  <div className="bg-white border border-gray-200 rounded-md p-3 mb-6 shadow-sm">
      ...
      {steps.map((label, index) => {
        ...
        return (
          <React.Fragment key={label}>
            <div className="flex flex-col items-center relative z-10" style={{ width: 56 }}>
              <div
              ...
              <div className={`mt-2 text-center font-medium ${isActive ? "text-[#7B3FE4]" : "text-gray-500"}`} style={{ fontSize: "10px" }}>
                {label}
              </div>
            </div>
            {index < steps.length - 1 && (
              <div className="flex-1 flex items-center justify-center">
                <div className="h-[1.5px] w-full bg-gray-200" style={{ maxWidth: 70 }} />
              </div>
            )}
          </React.Fragment>
        );
