...
export default function Restrictions({
  ...
  useEffect(() => {
    const fetchInitialData = async () => {
        // Handle Create Mode (LocalStorage)
        if (!campaignId || !isEditMode) {
             if (data && isFirstRun.current) {
                if (data.selectedCompanies) {
                    const companyIds = data.selectedCompanies.map(c => 
        ...
        // ✅ RECONSTRUCT DATA FASTLY FOR EDIT MODE
        setIsLoadingData(true);
        try {
            const res = await campaignDiscountApi.getById(campaignId);
            const d = res.data?.discount || {};
            ...
            if (restrictions.length > 0) {
                ...
                restrictions.forEach(item => {
                    newSelectedMIDs.add(item.brand_id);
                    if (item.all_tids) brandsWithAllTids.push(item.brand_id);
                    else if (item.discount_tids) {
                        newPerMerchantTerminals[item.brand_id] = item.discount_tids.map(t => t.terminal_id);
                    }
                });

                // ✅ FASTEST WAY: Parallel fetch all company brands at once to find matches
                const companyBrandsResults = await Promise.all(
                    companiesList.map(async (company) => {
                        const bRes = await metadataApi.getBrands(company.id);
                        return { companyId: company.id, brands: bRes.data?.rows || [] };
                    })
                );

                companyBrandsResults.forEach(result => {
                    if (result.brands.some(b => newSelectedMIDs.has(b.id))) {
                        companiesToSelect.add(result.companyId);
                    }
                });
                ...
                // Fetch full data for only the matched companies in parallel
                await Promise.all(finalCompanyIds.map(id => fetchMerchantsForCompany(id, brandsWithAllTids)));
            }
        } catch (err) {
            console.error("Failed to populate Step 4 details:", err);
        } finally {
            setIsLoadingData(false);
            isFirstRun.current = false;
        }
...
// useEffect(() => {
//     if (!isLoadingData && !isFirstRun.current) {
//         // Readable payload for summary view
//         const finalPayload = Array.from(selectedMIDs).map((brandId) => {
//             const merchant = allMerchants.find((m) => m.id === brandId);
//             const selectedTermIds = perMerchantTerminals[brandId] || [];
//             const totalTerms = merchant?.terminals?.length || 0;
...
//                 discount_tids: isAll ? [] : selectedTermIds.map((tid) => {
//                     const termObj = merchant?.terminals?.find(t => t.id === tid);
//                     return { terminal_id: tid, terminal_identifier: termObj?.label || `ID: ${tid}` };
//                 })
//             };
//         });
...
useEffect(() => {
  if (!isLoadingData && !isFirstRun.current) {
    // 1. Map through IDs to create a descriptive object for the UI
    const summaryData = Array.from(selectedMIDs).map((brandId) => {
      const merchant = allMerchants.find((m) => m.id === brandId);
      const selectedTermIds = perMerchantTerminals[brandId] || [];
      const totalTerms = merchant?.terminals?.length || 0;
      const isAll = totalTerms > 0 && selectedTermIds.length === totalTerms;
      ...
      return {
        brand_id: brandId,
        brand_name: merchant?.name || "Unknown Brand", // ✅ Added for summary
        all_tids: isAll,
        discount_tids: isAll ? [] : selectedTermIds.map((tid) => {
          const termObj = merchant?.terminals?.find(t => t.id === tid);
          return { 
            terminal_id: tid, 
            terminal_identifier: termObj?.label || `ID: ${tid}` // ✅ Added for summary
          };
        })
      };
    });

    // 2. Pass the descriptive data to props
    onUpdate({
      selectedCompanies,
      selectedMIDs: Array.from(selectedMIDs),
      perMerchantTerminals,
      finalMidRestrictions: summaryData 
    });
  }
}, [selectedMIDs, perMerchantTerminals, selectedCompanies, allMerchants, isLoadingData]);
...
const handleNextSubmit = async (action) => {
  ...
  const descriptiveList = Array.from(selectedMIDs).map((brandId) => {
    const selectedTerms = perMerchantTerminals[brandId] || [];
    const merchant = allMerchants.find((m) => m.id === brandId);
    const isAll = merchant?.terminals?.length > 0 && selectedTerms.length === merchant.terminals.length;
    ...
    return {
      brand_id: brandId,
      brand_name: merchant?.name || "Unknown Brand", // ✅ Included
      all_tids: isAll,
      discount_tids: isAll ? [] : selectedTerms.map((tid) => {
        const termObj = merchant?.terminals?.find(t => t.id === tid);
        return { terminal_id: tid, terminal_identifier: termObj?.label || `ID: ${tid}` }; // ✅ Included
      })
    };
  });
  ...
  const apiPayload = descriptiveList.map(item => ({
    brand_id: item.brand_id,
    all_tids: item.all_tids,
    discount_tids: item.discount_tids.map(t => ({ terminal_id: t.terminal_id })) // ✅ Clean IDs only
  }));
  ...
  onUpdate({
    ...
    selectedMIDs: Array.from(selectedMIDs),
    perMerchantTerminals,
    finalMidRestrictions: descriptiveList 
  });
  ...
  if (shouldCallApi) {
    try {
      ...
      const apiBody = {
        discount: { discount_mids: apiPayload }
      };
      ...
      console.error("Update failed:", err);
      const errorMessage = err.response?.data?.detail || "Failed to update restrictions.";
      Swal.fire({ icon: "error", title: "Error", text: errorMessage, confirmButtonColor: "#d33", background: "#FFFFFF" });
      return; 
    } finally {
      setIsUpdateSubmitting(false);
      setIsNextSubmitting(false);
  ...
  return (
    <div className="relative rounded-lg border border-gray-200 p-4 shadow-sm bg-[#F7F9FB] min-h-screen flex flex-col h-full">
       
       {/* ✅ FULL-SCREEN SPINNER FOR FAST RECONSTRUCTION */}
       {isLoadingData && (
        <div className="absolute inset-0 bg-white/80 z-[100] flex flex-col items-center justify-center rounded-lg">
          <Loader2 className="w-10 h-10 animate-spin text-[#7747EE] mb-4" />
          <h2 className="text-[#7747EE] font-bold text-lg">Rebuilding Restrictions List...</h2>
          <p className="text-gray-500 text-sm">Organizing companies, brands, and terminals</p>
        </div>
      )}
