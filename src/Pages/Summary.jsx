...
//       let netCount = 0;
//       if (step7.discount_card_networks && step7.discount_card_networks.length > 0) {
//           netCount = step7.discount_card_networks.length;
//       } else if (serverData?.discount?.discount_card_networks) {
//           netCount = serverData.discount.discount_card_networks.length;
//       }

//       let typeCount = 0;
//       if (step7.discount_card_types && step7.discount_card_types.length > 0) {
//           typeCount = step7.discount_card_types.length;
...
//               // Handle potential keys from Frontend (Step 2) OR Backend
//               // Frontend often uses: bin_ranges, apple_tokens
//               // Backend uses: discount_bins, discount_apple_tokens
//               const rawBins = s.bin_ranges || s.discount_bins || [];
//               const rawTokens = s.apple_tokens || s.discount_apple_tokens || [];

//               // 1. Map BINS to { start_bin, end_bin }
//               const discountBins = rawBins.map(b => ({
//                   start_bin: b.start_bin || b.start,
//                   end_bin: b.end_bin || b.end
//               }));

//               // 2. Map TOKENS to { token_value }
//               const discountTokens = rawTokens.map(t => ({
//                   // If 't' is a string (from frontend chips), map it to object.
//                   // If 't' is already object (from backend), use token_value or fallback.
//                   token_value: typeof t === 'string' ? t : (t.token_value || t.token)
//               }));

//               return { 
//                   segment_id: s.segment_id || s.id, 
//                   all_bins: s.all_bins ?? (discountBins.length === 0), // Use existing flag or infer false if bins exist
...
//           // CARD NETWORKS
//           let finalNetworks = [];
//           if (step7.discount_card_networks && Array.isArray(step7.discount_card_networks) && step7.discount_card_networks.length > 0) {
//               finalNetworks = step7.discount_card_networks;
//           } else if (serverData?.discount?.discount_card_networks) {
//               finalNetworks = serverData.discount.discount_card_networks.map(n => n.card_network_id || n.id);
//           }

//           // CARD TYPES
//           let finalTypes = [];
//           if (step7.discount_card_types && Array.isArray(step7.discount_card_types) && step7.discount_card_types.length > 0) {
//               finalTypes = step7.discount_card_types;
//           } else if (serverData?.discount?.discount_card_types) {
//               finalTypes = serverData.discount.discount_card_types.map(t => t.card_type_id || t.id);
//           }
...
//               discount: {
//                   discount_segments: finalSegments, // ✅ Key is strictly discount_segments
...
//             <span className="w-5 h-5 inline-flex items-center justify-center rounded-full" style={{ background: "#EFEFFD", color: "#7747EE", fontSize: 12 }}>8</span>
//             <h3 className="card-inside-head">Campaign Summary & Review</h3>
//           </div>
//           <div className="text-xs text-gray-500">Step 8 of 8</div>
//         </div>
  ...
  return (
    <div className="w-full max-w-6xl mx-auto">
      <div className="bg-white rounded-lg border border-gray-200 p-4 shadow-sm mb-6">
        ...
        <div className="flex items-start justify-between mb-4">
          <div className="flex gap-2 items-center">
            <span className="w-5 h-5 inline-flex items-center justify-center rounded-full" style={{ background: "#EFEFFD", color: "#7747EE", fontSize: 12 }}>9</span>
            <h3 className="card-inside-head">Campaign Summary & Review</h3>
          </div>
          <div className="text-xs text-gray-500">Step 9 of 9</div>
        </div>
